...
ggplot(SpecificCars, aes(KM, Prijs, color = Transmissie)) +
  geom_point(alpha=.1) +
  geom_smooth(method = "lm", formula = y ~ I(exp(x)), span=0.75, size=1.8) 
  facet_grid(~Merk+Model)+
scale_y_log10()
...
library(DT)
model_LinearDepreciation$depreciation = format(model_LinearDepreciation$depreciation , digits = 2) 
model_LinearDepreciation$Intercept = format(model_LinearDepreciation$Intercept , digits = 2, big.mark = ",") 

DT::datatable(model_LinearDepreciation)
